      ...
      <div className="mt-4">
        {pods.map((pod, index) => (
          ...
          >
            ...
            <div className="text-gray-300 w-28 truncate">{pod.id}</div>
            <div className="text-gray-300 w-40 truncate">{`${pod.date}| ${pod.time}`}</div>
            <div className="text-gray-300 w-28 truncate">{pod.carrier}</div>
            <div className="text-gray-300 w-32 truncate">{pod.route}</div>
            <div className="text-gray-300 w-20 truncate">
              {pod.cabinClass}
            </div>
            <div className="flex items-center gap-1.5">
              <div className="w-2 h-2 rounded-full bg-green-500"></div>
              <span className="text-gray-300">{pod.status}</span>
            </div>

            <div className="flex items-center gap-4">
              <div className="text-orange-400 font-medium">${pod.price}</div>
              <DropdownMenu
                open={activeMenuId === pod.id}
                onOpenChange={(open) =>
                  setActiveMenuId(open ? pod.id : null)
                }
              >
