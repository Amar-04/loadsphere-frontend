      ...
      <div className="mt-4">
        {bols.map((bol, index) => (
          ...
          >
            ...
            <div className="text-gray-300 w-28 truncate">{bol.id}</div>
            <div className="text-gray-300 w-40 truncate">{`${bol.date}| ${bol.time}`}</div>
            <div className="text-gray-300 w-28 truncate">{bol.carrier}</div>
            <div className="text-gray-300 w-32 truncate">{bol.route}</div>
            <div className="text-gray-300 w-20 truncate">
              {bol.cabinClass}
            </div>
            <div className="flex items-center gap-1.5">
              <div className="w-2 h-2 rounded-full bg-green-500"></div>
              <span className="text-gray-300">{bol.status}</span>
            </div>

            <div className="flex items-center gap-4">
              <div className="text-orange-400 font-medium">${bol.price}</div>
              <DropdownMenu
                open={activeMenuId === bol.id}
                onOpenChange={(open) =>
                  setActiveMenuId(open ? bol.id : null)
                }
              >
